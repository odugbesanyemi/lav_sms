            ...
            <div class="col-md-10">
                <form class="ajax-update" method="post" action="{{ route('ttr.update', $ttr->id) }}">
                    ...
                    <div class="form-group row">
                               ...
                               <div class="col-lg-9">
                                   <select class="form-control select" name="marking_period_id" id="marking_period_id">
                                       @foreach($markingPeriods as $mp)
                                           <option {{ $ttr->marking_period_id == $mp->id ? 'selected' : '' }} value="{{ $mp->id }}">{{ $mp->title }}</option>
                                       @endforeach
                                   </select>
                               </div>
                           </div>
                        ...
                        <div class="col-lg-9">
                            <select required data-placeholder="Select Class" class="form-control select" name="my_class_id" id="my_class_id">
                                @foreach($my_classes as $mc)
                                    <option {{ $ttr->my_class_id == $mc->id ? 'selected' : '' }} value="{{ $mc->id }}">{{ $mc->title}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
